...
export class RankingDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subseguimento = this.route.snapshot.paramMap.get('subseguimento')
    this.rankingPorAutomovel(this.id, this.subseguimento)
  }
 ...
 * @param mes 
 */
  rankingPorAutomovel(idt_auto: string, subseguimento: number): void {
    this.loading = !this.loading
    this.rankingService.rankingPorAutomovel(idt_auto, subseguimento).pipe(
      take(1)
    ).subscribe((ranking) => {
